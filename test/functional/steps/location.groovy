...
When(~'^ Usuário "([^"]*)" solicita equipamento "([^"]*)" para o laboratório "([^"]*)" $') { String user, Equipment equipment, String location ->
    requestEquipment(user, equipment, location);
}

Then(~'^Localização do equipamento "([^"]*)" é alterada para "([^"]*)"$') { Equipment equipment, String status ->
    modifyLocation(equipment, status)
}

Then(~'^Equipamento "([^"]*)" tem atributo encaminhamento alterado $') { Equipment equipment ->
